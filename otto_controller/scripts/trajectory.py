...
class Controller(Node):
    ...
    def timer_callback(self):
        ...
        if self.traj_vx != self.vx_d:
            ...
            if sig_vx == 1 and self.traj_vx > self.vx_d:
                self.traj_vx = self.vx_d
            elif sig_vx == -1 and self.traj_vx < self.vx_d:
                self.traj_vx = self.vx_d

        if self.traj_w != self.w_d:
            self.traj_w += sig_w * self.alpha * self.dt
            if sig_w == 1 and self.traj_w > self.w_d:
                self.traj_w = self.w_d
            elif sig_w == -1 and self.traj_w < self.w_d:
                self.traj_w = self.w_d

        if self.traj_vx >= 1.0 and self.traj_w >= 3.5:
            self.time += self.dt
        if self.time > 3:
            self.vx_d = 0.0
            self.w_d = 0.0
